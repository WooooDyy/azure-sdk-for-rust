...
impl TryFrom<(&Bytes,&HeaderMap)> for GetFileShareACLResponse{
    type Error = crate::Error;
    fn try_from((body,header_map):(&Bytes,&HeaderMap))-> Result<Self,Self::Error>{
        GetFileShareACLResponse::from_response(body,header_map)
    }
}

impl GetFileShareACLResponse{
    pub(crate) fn from_response(
        body:&Bytes,
        headers:&HeaderMap,
    )-> Result<GetFileShareACLResponse,crate::Error>{
